...
<script>
    ...
    $idInput.onkeyup = e => {
        ...
        if(idValue.trim() === ''){
            ...
            document.getElementById('idChk').innerHTML
                = '<b style = "color: red;">[아이디는 필수값입니다]';
            checkResultList[0] = false;
        } else if(!accountPattern.test(idValue)){
            $idInput.style.borderColor = 'red';
            document.getElementById('idChk').innerHTML
                = '<b style = "color: red;">[아이디는 4~14글자의 영문, 숫자로 입력하세요]';
            checkResultList[0] = false;
        } else {
            fetch('/members/check?type=account&value=' + idValue)
                .then(res => res.json())
                .then(flag => {
                    if(flag){ //중복
                        document.getElementById('idChk').innerHTML
                            = '<b style = "color: red;">[이미 사용중인 아이디입니다]';
                        checkResultList[0] = false;
                    }else { //중복 아님
                        $idInput.style.borderColor = 'skyblue';
                        document.getElementById('idChk').innerHTML
                            = '<b style = "color: skyblue;">[사용 가능한 아이디입니다]';
    ...
    $fileInput.onchange = e => {
        ...
        //파일의 바이트데이터를 읽어서 img태스의 src 속성이나 a태그의 href 속성에 넣기위한 형태로 읽음
